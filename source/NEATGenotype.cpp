...
//  default ctor
//------------------------------------------------------------------------
NEATGenotype::NEATGenotype():m_pPhenotype(NULL),
                   m_GenomeID(0),
                   m_dFitness(0),
...
//------------------------------------------------------------------------
NEATGenotype::NEATGenotype(int id, int inputs, int outputs):m_pPhenotype(NULL),
                                                  m_GenomeID(id),
                                                  m_dFitness(0),
                                                  m_dAdjustedFitness(0),
                                                  m_iNumInputs(inputs),
...
NEATGenotype::NEATGenotype(int id, int id2, int inputs, int outputs):m_pPhenotype(NULL),
                                                  m_GenomeID(id),
                                                  mID(id2),
                                                  m_dFitness(0),
                                                  m_dAdjustedFitness(0),
...
//------------------------------------------------------------------------
NEATGenotype::NEATGenotype(int                 id,
...
//
//---------------------------------------------------------------------------------------
NEATGenotype::NEATGenotype(const NEATGenotype& g)
{
...
//
//----------------------------------------------------------------------------------------
NEATGenotype& NEATGenotype::operator =(const NEATGenotype& g)
{
...
//------------------------------------------------------------------------
NEATNeuralNet* NEATGenotype::CreatePhenotype(int depth)
{
  ...
  m_pPhenotype = new NEATNeuralNet(vecNeurons, depth);
...
//
//------------------------------------------------------------------------
void NEATGenotype::DeletePhenotype()
{
  if (m_pPhenotype)
  {
    delete m_pPhenotype;
...
//  m_vecNeurons
//------------------------------------------------------------------------
int NEATGenotype::GetElementPos(int neuron_id)
{
  for (int i=0; i<m_vecNeurons.size(); i++)
	{
...
//  MessageBox(NULL, "Error in NEATGenotype::GetElementPos", "Problem!", MB_OK);
...
//------------------------------------------------------------------------
bool NEATGenotype::DuplicateLink(int NeuronIn, int NeuronOut)
{
	for (int cGene = 0; cGene < m_vecLinks.size(); ++cGene)
	{
...
//------------------------------------------------------------------------
void NEATGenotype::AddLink(double       MutationRate,
                      ...
                      int          NumTrysToAddLink)
{
  ...
  if (randFloat() < ChanceOfLooped)
  {
    ...
    while(NumTrysToFindLoop--)
    {
      ...
      int NeuronPos = rangedInt(m_iNumInputs+1, m_vecNeurons.size()-1);
  ...
  else
  {
    ...
    while(NumTrysToAddLink--)
    {
      ...
      ID_neuron1 = m_vecNeurons[rangedInt(0, m_vecNeurons.size()-1)].iID;

      ID_neuron2 =
      m_vecNeurons[rangedInt(m_iNumInputs+1, m_vecNeurons.size()-1)].iID;
...
//------------------------------------------------------------------------
void NEATGenotype::AddNeuron(double       MutationRate,
                        ...
                        int          NumTrysToFindOldLink)
{
  ...
  if (m_vecLinks.size() < SizeThreshold)
  {
    while(NumTrysToFindOldLink--)
    {
      ...
      ChosenLink = rangedInt(0, NumGenes()-1-(int)sqrt(NumGenes()));
  ...
  else
  {
    ...
    while (!bDone)
    {
      ChosenLink = rangedInt(0, NumGenes()-1);
  ...
  else
  {
    ...
    {
//      MessageBox(NULL, "Error in NEATGenotype::AddNeuron", "Problem!", MB_OK);
...
//------------------------------------------------------------------------
bool NEATGenotype::AlreadyHaveThisNeuronID(const int ID)
{
...
//------------------------------------------------------------------------
void NEATGenotype::MutateWeights(double mut_rate,
                            double prob_new_mut,
...
void NEATGenotype::MutateActivationResponse(double mut_rate,
                                       double MaxPertubation)
{
...
//------------------------------------------------------------------------
double NEATGenotype::GetCompatibilityScore(const NEATGenotype &genome)
{
